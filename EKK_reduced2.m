function [entry_pred, X_bar_pred] = EKK_reduced2(b)
...
[ delta_nF, X_nF, u_bar ] = EKK_simulator(S, m + 1, b);


N_nF_art  = sum(delta_nF); % when 0 predicted entry, 'NaN' appears.
entry_pred = mean(delta_nF);
entry_pred(1) = [];

X_bar_pred = sum(X_nF)./ N_nF_art;

X_bar_pred( isnan(X_bar_pred) ) = 0; % take 'NaN' as zero.
...
% k2 = kappa_2
        ...
        function [ delta_nF, X_nF, u_bar ] = EKK_simulator(S, mm, b)
       ...
       % lamb = param.lamb;
        ...
        u_bar_X = max( u_n_bar(:, 2:mm),[], 2 ); % S * 1 vector
        u_bar_F = u_n_bar(:, 1);
        ...
        %% step 8
        delta_nF = ( repmat(u, [1 mm] ) <= u_n_bar ) ; % n * m matrix
        
        component1 = repmat(u, [1 mm])  ./ u_n_bar ;
